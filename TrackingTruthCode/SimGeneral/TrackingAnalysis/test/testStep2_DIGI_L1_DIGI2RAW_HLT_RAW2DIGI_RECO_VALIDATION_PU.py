...
# with command line options: valTest --datatier GEN-SIM-RAW --conditions auto:startup -s DIGI,L1,DIGI2RAW,HLT,RAW2DIGI,RECO,VALIDATION --eventcontent FEVTDEBUG -n 5 --filein file:step1.root --fileout file:val.root --pileup LowLumiPileUp --pileup_input file:MinBias_60X.root --no_exec
...
process.FEVTDEBUGoutput = cms.OutputModule("PoolOutputModule",
    ...
    fileName = cms.untracked.string('file:val.root'),
...
#process.trackValidator.label_tp_effic = cms.InputTag("mergedtruth")
#process.trackValidator.label_tp_fake = cms.InputTag("mergedtruth")
#process.trackValidator.label_tv = cms.InputTag("mergedtruth")
...
process.FEVTDEBUGoutput.fileName = cms.untracked.string('file:ZEE_slimValidation.root')

process.trackingOnlyValidation = cms.Path( process.trackingTruthValid
	+process.tracksValidation
	+process.trackValidatorNew
)
process.schedule = cms.Schedule(process.digitisation_step,process.L1simulation_step,process.digi2raw_step)
process.schedule.extend(process.HLTSchedule)
process.schedule.extend([process.raw2digi_step,process.reconstruction_step,process.prevalidation_step,process.trackingOnlyValidation,process.endjob_step,process.FEVTDEBUGoutput_step])
