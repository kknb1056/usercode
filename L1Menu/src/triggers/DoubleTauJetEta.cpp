...
namespace l1menu
{
	namespace triggers
	{
		/** @brief Base class for all versions of the DoubleTauJetEta trigger.
		 *
		 ...
		 * @date 02/Jun/2013
		 */
		class DoubleTauJetEta : public l1menu::ITrigger
		{
		public:
			DoubleTauJetEta();
		...
		}; // end of the DoubleTauJetEta base class

		/** @brief First version of the DoubleTauJetEta trigger.
		 *
		 * @author probably Brian Winer
		 * @date sometime
		 */
		class DoubleTauJetEta_v0 : public DoubleTauJetEta
		{
		public:
		 ...
		 */
		REGISTER_TRIGGER_AND_CUSTOMISE( DoubleTauJetEta_v0,
			[]() // Use a lambda function to customise rather than creating a named function that never gets used again.
			{
				l1menu::TriggerTable& triggerTable=l1menu::TriggerTable::instance();
				DoubleTauJetEta_v0 tempTriggerInstance;
...
bool l1menu::triggers::DoubleTauJetEta_v0::apply( const l1menu::IEvent& event ) const
{
...
unsigned int l1menu::triggers::DoubleTauJetEta_v0::version() const
{
	return 0;
}

void l1menu::triggers::DoubleTauJetEta::initiateForReducedSample( const l1menu::ReducedMenuSample& sample )
{
	...
	if( iFindResult==parameterIdentifiers.end() ) throw std::runtime_error( "DoubleTauJetEta::initiateForReducedSample() - it appears this reduced sample wasn't created with this trigger. You can only run over a l1menu::ReducedMenuSample with triggers that were on when the sample was created." );
...
bool l1menu::triggers::DoubleTauJetEta::apply( const l1menu::IReducedEvent& event ) const
{
...
l1menu::triggers::DoubleTauJetEta::DoubleTauJetEta()
...
const std::string l1menu::triggers::DoubleTauJetEta::name() const
{
	return "L1_DoubleTau";
}

const std::vector<std::string> l1menu::triggers::DoubleTauJetEta::parameterNames() const
{
...
float& l1menu::triggers::DoubleTauJetEta::parameter( const std::string& parameterName )
{
...
const float& l1menu::triggers::DoubleTauJetEta::parameter( const std::string& parameterName ) const
{
	if( parameterName=="threshold1" ) return threshold1_;
	else if( parameterName=="threshold2" ) return threshold2_;
	else if( parameterName=="threshold3" ) return threshold3_;
