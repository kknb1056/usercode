...
namespace l1menu
{
	namespace triggers
	{
		/** @brief Base class for all versions of the Muer_HTM trigger.
		 *
		 ...
		 * @date 02/Jun/2013
		 */
		class Muer_HTM : public l1menu::ITrigger
		{
		public:
			Muer_HTM();
		...
		}; // end of the Muer_HTM base class

		/** @brief First version of the Muer_HTM trigger.
		 *
		 * @author probably Brian Winer
		 * @date sometime
		 */
		class Muer_HTM_v0 : public Muer_HTM
		{
		public:
...
bool l1menu::triggers::Muer_HTM_v0::apply( const l1menu::IEvent& event ) const
{
...
void l1menu::triggers::Muer_HTM::initiateForReducedSample( const l1menu::ReducedMenuSample& sample )
{
	...
	if( iFindResult==parameterIdentifiers.end() ) throw std::runtime_error( "Muer_HTM::initiateForReducedSample() - it appears this reduced sample wasn't created with this trigger. You can only run over a l1menu::ReducedMenuSample with triggers that were on when the sample was created." );
...
bool l1menu::triggers::Muer_HTM::apply( const l1menu::IReducedEvent& event ) const
{
...
l1menu::triggers::Muer_HTM::Muer_HTM()
...
const std::string l1menu::triggers::Muer_HTM::name() const
{
	return "L1_SingleMu_HTM";
}

const std::vector<std::string> l1menu::triggers::Muer_HTM::parameterNames() const
{
...
float& l1menu::triggers::Muer_HTM::parameter( const std::string& parameterName )
{
...
const float& l1menu::triggers::Muer_HTM::parameter( const std::string& parameterName ) const
{
	if( parameterName=="threshold1" ) return threshold1_;
	else if( parameterName=="threshold2" ) return threshold2_;
	else if( parameterName=="threshold3" ) return threshold3_;
