...
namespace l1menu
{
	namespace triggers
	{
		/** @brief Base class for all versions of the IsoEG_ETM trigger.
		 *
		 ...
		 * @date 02/Jun/2013
		 */
		class IsoEG_ETM : public l1menu::ITrigger
		{
		public:
			IsoEG_ETM();
		...
		}; // end of the IsoEG_ETM base class

		/** @brief First version of the IsoEG_ETM trigger.
		 *
		 * @author probably Brian Winer
		 * @date sometime
		 */
		class IsoEG_ETM_v0 : public IsoEG_ETM
		{
		public:
		 ...
		 */
		REGISTER_TRIGGER_AND_CUSTOMISE( IsoEG_ETM_v0,
			[]() // Use a lambda function to customise rather than creating a named function that never gets used again.
			{
				l1menu::TriggerTable& triggerTable=l1menu::TriggerTable::instance();
				IsoEG_ETM_v0 tempTriggerInstance;
...
bool l1menu::triggers::IsoEG_ETM_v0::apply( const l1menu::IEvent& event ) const
{
...
unsigned int l1menu::triggers::IsoEG_ETM_v0::version() const
{
	return 0;
}

void l1menu::triggers::IsoEG_ETM::initiateForReducedSample( const l1menu::ReducedMenuSample& sample )
{
	...
	if( iFindResult==parameterIdentifiers.end() ) throw std::runtime_error( "IsoEG_ETM::initiateForReducedSample() - it appears this reduced sample wasn't created with this trigger. You can only run over a l1menu::ReducedMenuSample with triggers that were on when the sample was created." );
...
bool l1menu::triggers::IsoEG_ETM::apply( const l1menu::IReducedEvent& event ) const
{
...
l1menu::triggers::IsoEG_ETM::IsoEG_ETM()
...
const std::string l1menu::triggers::IsoEG_ETM::name() const
{
	return "L1_SingleIsoEG_ETM";
}

const std::vector<std::string> l1menu::triggers::IsoEG_ETM::parameterNames() const
{
...
float& l1menu::triggers::IsoEG_ETM::parameter( const std::string& parameterName )
{
...
const float& l1menu::triggers::IsoEG_ETM::parameter( const std::string& parameterName ) const
{
	if( parameterName=="threshold1" ) return threshold1_;
	else if( parameterName=="threshold2" ) return threshold2_;
	else if( parameterName=="threshold3" ) return threshold3_;
