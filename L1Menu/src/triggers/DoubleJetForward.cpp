...
namespace l1menu
{
	namespace triggers
	{
		/** @brief Base class for all versions of the DoubleJetForward trigger.
		 *
		 ...
		 * @date 02/Jun/2013
		 */
		class DoubleJetForward : public l1menu::ITrigger
		{
		public:
			DoubleJetForward();
		...
		}; // end of the DoubleJetForward base class

		/** @brief First version of the DoubleJetForward trigger.
		 *
		 * @author probably Brian Winer
		 * @date sometime
		 */
		class DoubleJetForward_v0 : public DoubleJetForward
		{
		public:
		 ...
		 */
		REGISTER_TRIGGER_AND_CUSTOMISE( DoubleJetForward_v0,
			[]() // Use a lambda function to customise rather than creating a named function that never gets used again.
			{
				l1menu::TriggerTable& triggerTable=l1menu::TriggerTable::instance();
				DoubleJetForward_v0 tempTriggerInstance;
...
bool l1menu::triggers::DoubleJetForward_v0::apply( const l1menu::IEvent& event ) const
{
...
unsigned int l1menu::triggers::DoubleJetForward_v0::version() const
{
	return 0;
}

void l1menu::triggers::DoubleJetForward::initiateForReducedSample( const l1menu::ReducedMenuSample& sample )
{
	...
	if( iFindResult==parameterIdentifiers.end() ) throw std::runtime_error( "DoubleJetForward::initiateForReducedSample() - it appears this reduced sample wasn't created with this trigger. You can only run over a l1menu::ReducedMenuSample with triggers that were on when the sample was created." );
...
bool l1menu::triggers::DoubleJetForward::apply( const l1menu::IReducedEvent& event ) const
{
...
l1menu::triggers::DoubleJetForward::DoubleJetForward()
...
const std::string l1menu::triggers::DoubleJetForward::name() const
{
	return "L1_DoubleFwdJet";
}

const std::vector<std::string> l1menu::triggers::DoubleJetForward::parameterNames() const
{
...
float& l1menu::triggers::DoubleJetForward::parameter( const std::string& parameterName )
{
...
const float& l1menu::triggers::DoubleJetForward::parameter( const std::string& parameterName ) const
{
	if( parameterName=="threshold1" ) return threshold1_;
	else if( parameterName=="threshold2" ) return threshold2_;
	else if( parameterName=="threshold3" ) return threshold3_;
