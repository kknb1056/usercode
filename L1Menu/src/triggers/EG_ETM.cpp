...
namespace l1menu
{
	namespace triggers
	{
		/** @brief Base class for all versions of the EG_ETM trigger.
		 *
		 ...
		 * @date 02/Jun/2013
		 */
		class EG_ETM : public l1menu::ITrigger
		{
		public:
			EG_ETM();
		...
		}; // end of the EG_ETM base class

		/** @brief First version of the EG_ETM trigger.
		 *
		 * @author probably Brian Winer
		 * @date sometime
		 */
		class EG_ETM_v0 : public EG_ETM
		{
		public:
		 ...
		 */
		REGISTER_TRIGGER_AND_CUSTOMISE( EG_ETM_v0,
			[]() // Use a lambda function to customise rather than creating a named function that never gets used again.
			{
				l1menu::TriggerTable& triggerTable=l1menu::TriggerTable::instance();
				EG_ETM_v0 tempTriggerInstance;
...
bool l1menu::triggers::EG_ETM_v0::apply( const l1menu::IEvent& event ) const
{
...
unsigned int l1menu::triggers::EG_ETM_v0::version() const
{
	return 0;
}

void l1menu::triggers::EG_ETM::initiateForReducedSample( const l1menu::ReducedMenuSample& sample )
{
	...
	if( iFindResult==parameterIdentifiers.end() ) throw std::runtime_error( "EG_ETM::initiateForReducedSample() - it appears this reduced sample wasn't created with this trigger. You can only run over a l1menu::ReducedMenuSample with triggers that were on when the sample was created." );
...
bool l1menu::triggers::EG_ETM::apply( const l1menu::IReducedEvent& event ) const
{
...
const std::string l1menu::triggers::EG_ETM::name() const
{
	return "L1_SingleEG_ETM";
}

const std::vector<std::string> l1menu::triggers::EG_ETM::parameterNames() const
{
...
float& l1menu::triggers::EG_ETM::parameter( const std::string& parameterName )
{
...
const float& l1menu::triggers::EG_ETM::parameter( const std::string& parameterName ) const
{
	if( parameterName=="threshold1" ) return threshold1_;
	else if( parameterName=="threshold2" ) return threshold2_;
	else if( parameterName=="threshold3" ) return threshold3_;
