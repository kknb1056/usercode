...
namespace l1menu
{
	namespace triggers
	{
		/** @brief Base class for all versions of the EG_Mu trigger.
		 *
		 ...
		 * @date 02/Jun/2013
		 */
		class EG_Mu : public l1menu::ITrigger
		{
		public:
			EG_Mu();
		...
		}; // end of the EG_Mu base class

		/** @brief First version of the EG_Mu trigger.
		 *
		 * @author probably Brian Winer
		 * @date sometime
		 */
		class EG_Mu_v0 : public EG_Mu
		{
		public:
...
bool l1menu::triggers::EG_Mu_v0::apply( const l1menu::IEvent& event ) const
{
...
void l1menu::triggers::EG_Mu::initiateForReducedSample( const l1menu::ReducedMenuSample& sample )
{
	...
	if( iFindResult==parameterIdentifiers.end() ) throw std::runtime_error( "EG_Mu::initiateForReducedSample() - it appears this reduced sample wasn't created with this trigger. You can only run over a l1menu::ReducedMenuSample with triggers that were on when the sample was created." );
...
bool l1menu::triggers::EG_Mu::apply( const l1menu::IReducedEvent& event ) const
{
...
const std::string l1menu::triggers::EG_Mu::name() const
{
	return "L1_EG_Mu";
}

const std::vector<std::string> l1menu::triggers::EG_Mu::parameterNames() const
{
...
float& l1menu::triggers::EG_Mu::parameter( const std::string& parameterName )
{
...
const float& l1menu::triggers::EG_Mu::parameter( const std::string& parameterName ) const
{
	if( parameterName=="threshold1" ) return threshold1_;
	else if( parameterName=="threshold2" ) return threshold2_;
	else if( parameterName=="threshold3" ) return threshold3_;
