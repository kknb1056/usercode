...
namespace l1menu
{
	namespace triggers
	{
		/** @brief Base class for all versions of the Tau_HTM trigger.
		 *
		 ...
		 * @date 02/Jun/2013
		 */
		class Tau_HTM : public l1menu::ITrigger
		{
		public:
			Tau_HTM();
		...
		}; // end of the Tau_HTM base class

		/** @brief First version of the Tau_HTM trigger.
		 *
		 * @author probably Brian Winer
		 * @date sometime
		 */
		class Tau_HTM_v0 : public Tau_HTM
		{
		public:
		 ...
		 */
		REGISTER_TRIGGER_AND_CUSTOMISE( Tau_HTM_v0,
			[]() // Use a lambda function to customise rather than creating a named function that never gets used again.
			{
				l1menu::TriggerTable& triggerTable=l1menu::TriggerTable::instance();
				Tau_HTM_v0 tempTriggerInstance;
...
bool l1menu::triggers::Tau_HTM_v0::apply( const l1menu::IEvent& event ) const
{
...
unsigned int l1menu::triggers::Tau_HTM_v0::version() const
{
	return 0;
}

void l1menu::triggers::Tau_HTM::initiateForReducedSample( const l1menu::ReducedMenuSample& sample )
{
	...
	if( iFindResult==parameterIdentifiers.end() ) throw std::runtime_error( "Tau_HTM::initiateForReducedSample() - it appears this reduced sample wasn't created with this trigger. You can only run over a l1menu::ReducedMenuSample with triggers that were on when the sample was created." );
...
bool l1menu::triggers::Tau_HTM::apply( const l1menu::IReducedEvent& event ) const
{
...
l1menu::triggers::Tau_HTM::Tau_HTM()
...
const std::string l1menu::triggers::Tau_HTM::name() const
{
	return "L1_SingleTau_HTM";
}

const std::vector<std::string> l1menu::triggers::Tau_HTM::parameterNames() const
{
...
float& l1menu::triggers::Tau_HTM::parameter( const std::string& parameterName )
{
...
const float& l1menu::triggers::Tau_HTM::parameter( const std::string& parameterName ) const
{
	if( parameterName=="threshold1" ) return threshold1_;
	else if( parameterName=="threshold2" ) return threshold2_;
	else if( parameterName=="threshold3" ) return threshold3_;
