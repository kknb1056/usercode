...
namespace l1menu
{
	namespace triggers
	{
		/** @brief Base class for all versions of the Mu_isoEG trigger.
		 *
		 ...
		 * @date 02/Jun/2013
		 */
		class Mu_isoEG : public l1menu::ITrigger
		{
		public:
			Mu_isoEG();
		...
		}; // end of the Mu_isoEG base class

		/** @brief First version of the Mu_isoEG trigger.
		 *
		 * @author probably Brian Winer
		 * @date sometime
		 */
		class Mu_isoEG_v0 : public Mu_isoEG
		{
		public:
		 ...
		 */
		REGISTER_TRIGGER_AND_CUSTOMISE( Mu_isoEG_v0,
			[]() // Use a lambda function to customise rather than creating a named function that never gets used again.
			{
				l1menu::TriggerTable& triggerTable=l1menu::TriggerTable::instance();
				Mu_isoEG_v0 tempTriggerInstance;
...
bool l1menu::triggers::Mu_isoEG_v0::apply( const l1menu::IEvent& event ) const
{
...
unsigned int l1menu::triggers::Mu_isoEG_v0::version() const
{
	return 0;
}

void l1menu::triggers::Mu_isoEG::initiateForReducedSample( const l1menu::ReducedMenuSample& sample )
{
	...
	if( iFindResult==parameterIdentifiers.end() ) throw std::runtime_error( "Mu_isoEG::initiateForReducedSample() - it appears this reduced sample wasn't created with this trigger. You can only run over a l1menu::ReducedMenuSample with triggers that were on when the sample was created." );
...
bool l1menu::triggers::Mu_isoEG::apply( const l1menu::IReducedEvent& event ) const
{
...
l1menu::triggers::Mu_isoEG::Mu_isoEG()
...
const std::string l1menu::triggers::Mu_isoEG::name() const
{
	return "L1_isoMu_isoEG";
}

const std::vector<std::string> l1menu::triggers::Mu_isoEG::parameterNames() const
{
...
float& l1menu::triggers::Mu_isoEG::parameter( const std::string& parameterName )
{
...
const float& l1menu::triggers::Mu_isoEG::parameter( const std::string& parameterName ) const
{
	if( parameterName=="threshold1" ) return threshold1_;
	else if( parameterName=="threshold2" ) return threshold2_;
	else if( parameterName=="threshold3" ) return threshold3_;
