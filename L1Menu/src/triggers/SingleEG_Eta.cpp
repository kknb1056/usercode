...
namespace l1menu
{
	namespace triggers
	{
		/** @brief Base class for all versions of the SingleEG_Eta trigger.
		 *
		 ...
		 * @date 02/Jun/2013
		 */
		class SingleEG_Eta : public l1menu::ITrigger
		{
		public:
			SingleEG_Eta();
		...
		}; // end of the SingleEG_Eta base class

		/** @brief First version of the SingleEG_Eta trigger.
		 *
		 * @author probably Brian Winer
		 * @date sometime
		 */
		class SingleEG_Eta_v0 : public SingleEG_Eta
		{
		public:
		 ...
		 */
		REGISTER_TRIGGER_AND_CUSTOMISE( SingleEG_Eta_v0,
			[]() // Use a lambda function to customise rather than creating a named function that never gets used again.
			{
				l1menu::TriggerTable& triggerTable=l1menu::TriggerTable::instance();
				SingleEG_Eta_v0 tempTriggerInstance;
...
bool l1menu::triggers::SingleEG_Eta_v0::apply( const l1menu::IEvent& event ) const
{
...
unsigned int l1menu::triggers::SingleEG_Eta_v0::version() const
{
	return 0;
}

void l1menu::triggers::SingleEG_Eta::initiateForReducedSample( const l1menu::ReducedMenuSample& sample )
{
	...
	if( iFindResult==parameterIdentifiers.end() ) throw std::runtime_error( "SingleEG_Eta::initiateForReducedSample() - it appears this reduced sample wasn't created with this trigger. You can only run over a l1menu::ReducedMenuSample with triggers that were on when the sample was created." );
...
bool l1menu::triggers::SingleEG_Eta::apply( const l1menu::IReducedEvent& event ) const
{
...
l1menu::triggers::SingleEG_Eta::SingleEG_Eta()
...
const std::string l1menu::triggers::SingleEG_Eta::name() const
{
	return "L1_SingleEG";
}

const std::vector<std::string> l1menu::triggers::SingleEG_Eta::parameterNames() const
{
...
float& l1menu::triggers::SingleEG_Eta::parameter( const std::string& parameterName )
{
...
const float& l1menu::triggers::SingleEG_Eta::parameter( const std::string& parameterName ) const
{
	if( parameterName=="threshold1" ) return threshold1_;
	else if( parameterName=="threshold2" ) return threshold2_;
	else if( parameterName=="threshold3" ) return threshold3_;
