...
namespace l1menu
{
	namespace triggers
	{
		/** @brief Base class for all versions of the SingleIsoEG_Eta trigger.
		 *
		 ...
		 * @date 02/Jun/2013
		 */
		class SingleIsoEG_Eta : public l1menu::ITrigger
		{
		public:
			SingleIsoEG_Eta();
		...
		}; // end of the SingleIsoEG_Eta base class

		/** @brief First version of the SingleIsoEG_Eta trigger.
		 *
		 * @author probably Brian Winer
		 * @date sometime
		 */
		class SingleIsoEG_Eta_v0 : public SingleIsoEG_Eta
		{
		public:
		 ...
		 */
		REGISTER_TRIGGER_AND_CUSTOMISE( SingleIsoEG_Eta_v0,
			[]() // Use a lambda function to customise rather than creating a named function that never gets used again.
			{
				l1menu::TriggerTable& triggerTable=l1menu::TriggerTable::instance();
				SingleIsoEG_Eta_v0 tempTriggerInstance;
...
bool l1menu::triggers::SingleIsoEG_Eta_v0::apply( const l1menu::IEvent& event ) const
{
...
unsigned int l1menu::triggers::SingleIsoEG_Eta_v0::version() const
{
	return 0;
}

void l1menu::triggers::SingleIsoEG_Eta::initiateForReducedSample( const l1menu::ReducedMenuSample& sample )
{
	...
	if( iFindResult==parameterIdentifiers.end() ) throw std::runtime_error( "SingleIsoEG_Eta::initiateForReducedSample() - it appears this reduced sample wasn't created with this trigger. You can only run over a l1menu::ReducedMenuSample with triggers that were on when the sample was created." );
...
bool l1menu::triggers::SingleIsoEG_Eta::apply( const l1menu::IReducedEvent& event ) const
{
...
l1menu::triggers::SingleIsoEG_Eta::SingleIsoEG_Eta()
...
const std::string l1menu::triggers::SingleIsoEG_Eta::name() const
{
	return "L1_SingleIsoEG";
}

const std::vector<std::string> l1menu::triggers::SingleIsoEG_Eta::parameterNames() const
{
...
float& l1menu::triggers::SingleIsoEG_Eta::parameter( const std::string& parameterName )
{
...
const float& l1menu::triggers::SingleIsoEG_Eta::parameter( const std::string& parameterName ) const
{
	if( parameterName=="threshold1" ) return threshold1_;
	else if( parameterName=="threshold2" ) return threshold2_;
	else if( parameterName=="threshold3" ) return threshold3_;
