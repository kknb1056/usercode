...
bool l1menu::triggers::SingleTauJet_v0::apply( const l1menu::IEvent& event ) const
{
...
void l1menu::triggers::SingleTauJet::initiateForReducedSample( const l1menu::ReducedMenuSample& sample )
{
	...
	if( iFindResult==parameterIdentifiers.end() ) throw std::runtime_error( "SingleTauJet::initiateForReducedSample() - it appears this reduced sample wasn't created with this trigger. You can only run over a l1menu::ReducedMenuSample with triggers that were on when the sample was created." );
...
bool l1menu::triggers::SingleTauJet::apply( const l1menu::IReducedEvent& event ) const
{
