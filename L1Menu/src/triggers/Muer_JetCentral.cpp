...
namespace l1menu
{
	namespace triggers
	{
		/** @brief Base class for all versions of the Muer_JetCentral trigger.
		 *
		 ...
		 * @date 02/Jun/2013
		 */
		class Muer_JetCentral : public l1menu::ITrigger
		{
		public:
			Muer_JetCentral();
		...
		}; // end of the Muer_JetCentral base class

		/** @brief First version of the Muer_JetCentral trigger.
		 *
		 * @author probably Brian Winer
		 * @date sometime
		 */
		class Muer_JetCentral_v0 : public Muer_JetCentral
		{
		public:
...
bool l1menu::triggers::Muer_JetCentral_v0::apply( const l1menu::IEvent& event ) const
{
...
void l1menu::triggers::Muer_JetCentral::initiateForReducedSample( const l1menu::ReducedMenuSample& sample )
{
	...
	if( iFindResult==parameterIdentifiers.end() ) throw std::runtime_error( "Muer_JetCentral::initiateForReducedSample() - it appears this reduced sample wasn't created with this trigger. You can only run over a l1menu::ReducedMenuSample with triggers that were on when the sample was created." );
...
bool l1menu::triggers::Muer_JetCentral::apply( const l1menu::IReducedEvent& event ) const
{
...
l1menu::triggers::Muer_JetCentral::Muer_JetCentral()
...
const std::string l1menu::triggers::Muer_JetCentral::name() const
{
	return "L1_SingleMu_CJet";
}

const std::vector<std::string> l1menu::triggers::Muer_JetCentral::parameterNames() const
{
...
float& l1menu::triggers::Muer_JetCentral::parameter( const std::string& parameterName )
{
...
const float& l1menu::triggers::Muer_JetCentral::parameter( const std::string& parameterName ) const
{
	if( parameterName=="threshold1" ) return threshold1_;
	else if( parameterName=="threshold2" ) return threshold2_;
	else if( parameterName=="threshold3" ) return threshold3_;
