...
namespace l1menu
{
	namespace triggers
	{
		/** @brief Base class for all versions of the Mu_Tau trigger.
		 *
		 ...
		 * @date 02/Jun/2013
		 */
		class Mu_Tau : public l1menu::ITrigger
		{
		public:
			Mu_Tau();
		...
		}; // end of the Mu_Tau base class

		/** @brief First version of the Mu_Tau trigger.
		 *
		 * @author probably Brian Winer
		 * @date sometime
		 */
		class Mu_Tau_v0 : public Mu_Tau
		{
		public:
...
bool l1menu::triggers::Mu_Tau_v0::apply( const l1menu::IEvent& event ) const
{
...
void l1menu::triggers::Mu_Tau::initiateForReducedSample( const l1menu::ReducedMenuSample& sample )
{
	...
	if( iFindResult==parameterIdentifiers.end() ) throw std::runtime_error( "Mu_Tau::initiateForReducedSample() - it appears this reduced sample wasn't created with this trigger. You can only run over a l1menu::ReducedMenuSample with triggers that were on when the sample was created." );
...
bool l1menu::triggers::Mu_Tau::apply( const l1menu::IReducedEvent& event ) const
{
...
const std::string l1menu::triggers::Mu_Tau::name() const
{
	return "L1_Mu_Tau";
}

const std::vector<std::string> l1menu::triggers::Mu_Tau::parameterNames() const
{
...
float& l1menu::triggers::Mu_Tau::parameter( const std::string& parameterName )
{
...
const float& l1menu::triggers::Mu_Tau::parameter( const std::string& parameterName ) const
{
	if( parameterName=="threshold1" ) return threshold1_;
	else if( parameterName=="threshold2" ) return threshold2_;
	else if( parameterName=="threshold3" ) return threshold3_;
