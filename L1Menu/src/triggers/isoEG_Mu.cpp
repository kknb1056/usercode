...
namespace l1menu
{
	namespace triggers
	{
		/** @brief Base class for all versions of the isoEG_Mu trigger.
		 *
		 ...
		 * @date 02/Jun/2013
		 */
		class isoEG_Mu : public l1menu::ITrigger
		{
		public:
			isoEG_Mu();
		...
		}; // end of the isoEG_Mu base class

		/** @brief First version of the isoEG_Mu trigger.
		 *
		 * @author probably Brian Winer
		 * @date sometime
		 */
		class isoEG_Mu_v0 : public isoEG_Mu
		{
		public:
		 ...
		 */
		REGISTER_TRIGGER_AND_CUSTOMISE( isoEG_Mu_v0,
			[]() // Use a lambda function to customise rather than creating a named function that never gets used again.
			{
				l1menu::TriggerTable& triggerTable=l1menu::TriggerTable::instance();
				isoEG_Mu_v0 tempTriggerInstance;
...
bool l1menu::triggers::isoEG_Mu_v0::apply( const l1menu::IEvent& event ) const
{
...
unsigned int l1menu::triggers::isoEG_Mu_v0::version() const
{
	return 0;
}

void l1menu::triggers::isoEG_Mu::initiateForReducedSample( const l1menu::ReducedMenuSample& sample )
{
	...
	if( iFindResult==parameterIdentifiers.end() ) throw std::runtime_error( "isoEG_Mu::initiateForReducedSample() - it appears this reduced sample wasn't created with this trigger. You can only run over a l1menu::ReducedMenuSample with triggers that were on when the sample was created." );
...
bool l1menu::triggers::isoEG_Mu::apply( const l1menu::IReducedEvent& event ) const
{
...
l1menu::triggers::isoEG_Mu::isoEG_Mu()
...
const std::string l1menu::triggers::isoEG_Mu::name() const
{
	return "L1_isoEG_isoMu";
}

const std::vector<std::string> l1menu::triggers::isoEG_Mu::parameterNames() const
{
...
float& l1menu::triggers::isoEG_Mu::parameter( const std::string& parameterName )
{
...
const float& l1menu::triggers::isoEG_Mu::parameter( const std::string& parameterName ) const
{
	if( parameterName=="threshold1" ) return threshold1_;
	else if( parameterName=="threshold2" ) return threshold2_;
	else if( parameterName=="threshold3" ) return threshold3_;
