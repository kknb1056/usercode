...
namespace l1menu
{
	namespace triggers
	{
		/** @brief Base class for all versions of the Tau_TwoJetForward trigger.
		 *
		 ...
		 * @date 02/Jun/2013
		 */
		class Tau_TwoJetForward : public l1menu::ITrigger
		{
		public:
			Tau_TwoJetForward();
		...
		}; // end of the Tau_TwoJetForward base class

		/** @brief First version of the Tau_TwoJetForward trigger.
		 *
		 * @author probably Brian Winer
		 * @date sometime
		 */
		class Tau_TwoJetForward_v0 : public Tau_TwoJetForward
		{
		public:
		 ...
		 */
		REGISTER_TRIGGER_AND_CUSTOMISE( Tau_TwoJetForward_v0,
			[]() // Use a lambda function to customise rather than creating a named function that never gets used again.
			{
				l1menu::TriggerTable& triggerTable=l1menu::TriggerTable::instance();
				Tau_TwoJetForward_v0 tempTriggerInstance;
...
bool l1menu::triggers::Tau_TwoJetForward_v0::apply( const l1menu::IEvent& event ) const
{
...
unsigned int l1menu::triggers::Tau_TwoJetForward_v0::version() const
{
	return 0;
}

void l1menu::triggers::Tau_TwoJetForward::initiateForReducedSample( const l1menu::ReducedMenuSample& sample )
{
	...
	if( iFindResult==parameterIdentifiers.end() ) throw std::runtime_error( "Tau_TwoJetForward::initiateForReducedSample() - it appears this reduced sample wasn't created with this trigger. You can only run over a l1menu::ReducedMenuSample with triggers that were on when the sample was created." );
...
bool l1menu::triggers::Tau_TwoJetForward::apply( const l1menu::IReducedEvent& event ) const
{
...
l1menu::triggers::Tau_TwoJetForward::Tau_TwoJetForward()
...
const std::string l1menu::triggers::Tau_TwoJetForward::name() const
{
	return "L1_SingleTau_TwoFJet";
}

const std::vector<std::string> l1menu::triggers::Tau_TwoJetForward::parameterNames() const
{
...
float& l1menu::triggers::Tau_TwoJetForward::parameter( const std::string& parameterName )
{
...
const float& l1menu::triggers::Tau_TwoJetForward::parameter( const std::string& parameterName ) const
{
	if( parameterName=="threshold1" ) return threshold1_;
	else if( parameterName=="threshold2" ) return threshold2_;
	else if( parameterName=="threshold3" ) return threshold3_;
