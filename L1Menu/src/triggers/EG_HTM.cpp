...
namespace l1menu
{
	namespace triggers
	{
		/** @brief Base class for all versions of the EG_HTM trigger.
		 *
		 ...
		 * @date 02/Jun/2013
		 */
		class EG_HTM : public l1menu::ITrigger
		{
		public:
			EG_HTM();
		...
		}; // end of the EG_HTM base class

		/** @brief First version of the EG_HTM trigger.
		 *
		 * @author probably Brian Winer
		 * @date sometime
		 */
		class EG_HTM_v0 : public EG_HTM
		{
		public:
...
bool l1menu::triggers::EG_HTM_v0::apply( const l1menu::IEvent& event ) const
{
...
void l1menu::triggers::EG_HTM::initiateForReducedSample( const l1menu::ReducedMenuSample& sample )
{
	...
	if( iFindResult==parameterIdentifiers.end() ) throw std::runtime_error( "EG_HTM::initiateForReducedSample() - it appears this reduced sample wasn't created with this trigger. You can only run over a l1menu::ReducedMenuSample with triggers that were on when the sample was created." );
...
bool l1menu::triggers::EG_HTM::apply( const l1menu::IReducedEvent& event ) const
{
...
const std::string l1menu::triggers::EG_HTM::name() const
{
	return "L1_SingleEG_HTM";
}

const std::vector<std::string> l1menu::triggers::EG_HTM::parameterNames() const
{
...
float& l1menu::triggers::EG_HTM::parameter( const std::string& parameterName )
{
...
const float& l1menu::triggers::EG_HTM::parameter( const std::string& parameterName ) const
{
	if( parameterName=="threshold1" ) return threshold1_;
	else if( parameterName=="threshold2" ) return threshold2_;
	else if( parameterName=="threshold3" ) return threshold3_;
