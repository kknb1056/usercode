...
int main( int argc, char* argv[] )
{
	...
	coords.push_back( std::make_pair( 3, 2.99*slope+intercept ) );
	coords.push_back( std::make_pair( 2.45, 2.4*slope+intercept ) );
	coords.push_back( std::make_pair( 23.23, 21.23*slope+intercept ) );
	coords.push_back( std::make_pair( 0.145, 0.125*slope+intercept ) );
	coords.push_back( std::make_pair( 0, 0*slope+intercept ) );
	coords.push_back( std::make_pair( -23, -22*slope+intercept ) );
	std::pair<float,float> slopeInterceptPair=l1menu::tools::simpleLinearFit( coords );
	std::cout << "slope=" << slopeInterceptPair.first << " intercept=" << slopeInterceptPair.second << std::endl;

	std::unique_ptr<l1menu::ITrigger> trigger=l1menu::TriggerTable::instance().getTrigger( "L1_HTT" );
